...
namespace LifeQuality.Test
{
    ...
    public class BloodAndAnalysisServiceTests
    {
        ...
        public async Task CreateAnalysisDataAsync_ValidSensorId_AnalyticsServiceCalled()
        {
            ...
            analyticsServiceMock.Setup(s => s.AnalyseReceivedDataAsync(sensorId)).ReturnsAsync(analyzedData);

            service.CreateAnalysisDataAsync(sensorId);

            analyticsServiceMock.Verify(s => s.AnalyseReceivedDataAsync(sensorId), Times.Once);
